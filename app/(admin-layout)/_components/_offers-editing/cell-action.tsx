    ...
    startTransition(() => {
      deleteOffer(data.id)
        .then((data) => {
          ...
          if (data.success) {
            router.refresh();
            toast.success("Офер було видалено");
          }
        })
    ...
    startTransition(() => {
      acceptOffer({ offerId: data.id, userId: user?.id || "" })
        .then((data) => {
          ...
          if (data.success) {
            router.refresh();
            toast.success("Ви прийняли офер");
          }
        })
        .catch(() => toast.error("Щось пішло не так!"));
    });
    ...
    startTransition(() => {
      declineOffer({ offerId: data.id, userId: user?.id || "" })
        .then((data) => {
          ...
          if (data.success) {
            router.refresh();
            toast.success("Ви відмовилися від офера");
          }
        })
        .catch(() => toast.error("Щось пішло не так!"));
    });
    ...
    <>
      ...
      {user && user.role == "AFFILIATE" ? (
        <div className="flex flex-row gap-3 justify-start items-center">
          {!data.accepted && data.authorId !== user.id && (
            <div
              onClick={() => acceptOfferCommand()}
              className="cursor-pointer bg-white rounded-full p-1
           flex items-center justify-center" /* bg-[#E7F7EF]h-[20px] w-[20px] */
            >
              <Tooltip message="Прийняти">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 28 28"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-[26px] h-[26px] pt-[3px]  stroke-[#0FAF62]"
                >
              ...
              >
                <path
                  d="M10.4766 3.95605L5.22656 9.20582L2.60156 6.58106"
                  stroke="#0FAF62"
                  strokeWidth="1.25"
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          {data.accepted && (
            <div
              onClick={() => declineOfferCommand()}
              className="cursor-pointer bg-white rounded-full p-1
           flex items-center justify-center" /* bg-[#FDEDED]  h-[20px] w-[20px]*/
            >
              <Tooltip message="Відмовитися">
                {/*  <svg
                width="20"
                height="20"
                viewBox="0 0 28 28"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="w-[26px] h-[26px] pl-[2px] stroke-[#E84646]"
              >
                <circle cx="13" cy="13" r="12" strokeWidth="2" />
                ...
                <svg
                  ...
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-[26px] h-[26px] pt-[3px] stroke-[#E84646]"
                >
                  <circle cx="5" cy="5" r="7" strokeWidth="1" />
                  <path
                    d="M7.38281 2.5498L2.32031 7.6123"
                    stroke="#E84646"
                    strokeWidth="1.25"
                    strokeLinecap="round"
                    strokeLinejoin="round"
